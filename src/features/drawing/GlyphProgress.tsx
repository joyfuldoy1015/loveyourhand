...
export function GlyphProgress({ glyphs, currentIndex, onSelect }: Props) {
  const completed = glyphs.filter((g) => g.isComplete).length;
  const pct = Math.round((completed / glyphs.length) * 100);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-[10px] font-medium tabular-nums text-[#6B6B6B] w-12 text-right">
